# 学习笔记索引（2026-03-21）

## 📊 今日统计

- **总笔记数**: 38 篇
- **Fork 项目**: 12 个
- **收集工具**: 90+
- **整理推文**: 60+

---

## 🔥 核心项目

### **1. MAS Factory**
- [快速开始](./MASFACTORY_QUICKSTART.md)
- [深度研究](./MASFACTORY_DEEP_DIVE.md)
- [学习链接](./MASFACTORY_LEARNING_LINK.md)
- [工作流配置](./masfactory-workflow/SKILL.md)

### **2. MiroFish**
- [群体智能引擎](./mirofish-swarm-intelligence.md)
- [预测赚钱案例](./polymarket-arbitrage-case.md)

### **3. OpenViking**
- [使用指南](./openviking-usage-guide.md)
- [OpenClaw 集成](./openviking-openclaw-integration.md)
- [z.ai 配置](./openviking-zai-config.md)

### **4. OpenSage**
- [自编程智能体引擎](./opensage-self-programming-agent.md)
- [论文解读](https://arxiv.org/abs/2602.16891)

### **5. MagicSkills**
- [Skill 基础设施](./magicskills-infrastructure.md)

---

## 🛠️ 工具生态

### **工具清单**
- [90个 AI 工具列表](./90-ai-tools-list.md)
- [Skills Manager](./skills-manager-unified-management.md)
- [Polymarket 交易生态](./polymarket-trading-ecosystem.md)

### **开发工具**
- [Helius - Rust 财务追踪](./helius-rust-finance-tracker.md)
- [Lightpanda - AI 浏览器](./lightpanda-ai-browser.md)
- [SearXNG - 私密搜索](./searxng-private-search.md)
- [CRTUI - 容器工具](./crtui-container-registry-tool.md)

---

## 💡 学习方法论

### **Vibe Coding**
- [工具全景](./vibe-coding-tools-overview.md)
- [工具总结](./vibe-coding-tools-summary.md)
- [案例研究](./vibe-coding-case-study.md)
- [TDD 方法](./vibe-tdd-approach.md)

### **学习方法论**
- [AI 学习方法论](./ai-learning-methodology.md)
- [代码即语言](./code-as-natural-language.md)
- [Karpathy 多 Agent 协作](./karpathy-multi-agent-method.md)

---

## 🔬 前沿研究

### **记忆系统**
- [MSA - 稀疏注意力记忆](./msa-memory-sparse-attention-deep.md)
- [OpenViking - 分层记忆](./openviking-usage-guide.md)
- [Agent 记忆对比](./agent-memory-research.md)

### **多智能体**
- [TradingAgents - 30.5% 年化](./tradingagents-system.md)
- [Agency-agents - 147个 Agent](./agency-agents-147-agents.md)

---

## 💼 实战案例

### **交易案例**
- [Polymarket 套利](./polymarket-arbitrage-case.md)
- [MiroFish 预测](./mirofish-swarm-intelligence.md)
- [TradingAgents 系统](./tradingagents-system.md)

### **开发案例**
- [响马 - 代码即 Prompt](./code-as-natural-language.md)
- [Google AI Studio 全栈](./google-ai-studio-fullstack-vibe.md)
- [Google AI Studio 产品阶段](./google-ai-studio-product-stage.md)

---

## 📚 学习资源

### **课程和文档**
- [Agent Skills 官方课程](./agent-skills-course.md)
- [Claude Code 技能手册](./claude-code-skills-manual.md)
- [Claude 全能 Prompt](./claude-all-rounder-prompt.md)
- [Codex Autoresearch 指南](./codex-autoresearch-guide.md)

### **设计资源**
- [Figma UI MCP](./figma-ui-mcp.md)
- [DesignPrompts UI 模板](./designprompts-ui-templates.md)

---

## 🔗 其他资源

### **API 和服务**
- [AxonHub AI Gateway](./axonhub-ai-gateway.md)
- [免费公开 API](./free-public-apis.md)
- [Google Maps API 免费](./final-four-resources.md)

### **安全和工具**
- [OpenClaw 安全指南](./openclaw-security-guide.md)
- [Workflow 演进](./workflow-evolution.md)
- [SkillCraft 演化](./skillcraft-evolution.md)

---

## 🎯 按优先级分类

### **⭐⭐⭐ 高优先级（必读）**
1. MAS Factory 快速开始
2. OpenSage 自编程引擎
3. MiroFish 群体智能
4. OpenViking 记忆系统
5. Vibe Coding 方法论

### **⭐⭐ 中优先级（推荐）**
1. 90个 AI 工具清单
2. Polymarket 套利案例
3. TradingAgents 系统
4. MagicSkills 基础设施
5. 代码即 Prompt

### **⭐ 低优先级（扩展）**
1. 各种工具和平台
2. 设计资源
3. API 服务
4. 安全指南

---

## 🔄 Fork 项目清单

### **核心项目（6个）**
1. **MiroFish** - https://github.com/srxly888-creator/MiroFish (37,844 Stars)
2. **MAS Factory** - https://github.com/srxly888-creator/MASFactory (125 Stars)
3. **OpenViking** - https://github.com/srxly888-creator/OpenViking (17,338 Stars)
4. **MagicSkills** - https://github.com/srxly888-creator/MagicSkills (58 Stars)
5. **Polymarket Builders** - https://github.com/srxly888-creator/awesome-polymarket-builders
6. **OpenSage** - （待 Fork，如有代码）

### **工具项目（6个）**
7. **agency-agents** - https://github.com/srxly888-creator/agency-agents (46.8k Stars)
8. **learn-claude-code** - https://github.com/srxly888-creator/learn-claude-code
9. **skillgrade** - https://github.com/srxly888-creator/skillgrade
10. **page-agent** - https://github.com/srxly888-creator/page-agent (9.6k Stars)
11. **pua** - https://github.com/srxly888-creator/pua (8.9k Stars)
12. **chrome-cdp-skill** - https://github.com/srxly888-creator/chrome-cdp-skill

---

## 📈 中国开源力量

### **占比**: 60%+

| 项目 | Stars | 团队 | 特点 |
|------|-------|------|------|
| **MiroFish** | 37,844 | 北邮 | 10天完成，已在赚钱 |
| **OpenViking** | 17,338 | 字节 | Token 砍半 |
| **TradingAgents** | 30,000+ | 中国 | 30.5% 年化 |
| **MAS Factory** | 125 | 北邮 | 多智能体编排 |
| **MagicSkills** | 58 | 北大 | Skill 基础设施 |
| **OpenSage** | - | 中国 | ICML 2026 |
| **PageAgent** | 9,600 | 阿里 | 网页 AI 化 |
| **agency-agents** | 46,800 | 中国 | 147个 Agent |

---

## 💬 金句合集

> **AI 没给你想要的结果，不是 AI 不行，是你还没找到把它串起来的方法**

> **从 Human-centered 到 AI-centered**

> **10天完成 37k Stars 项目，中国学生震撼世界**

> **4B 参数干翻 235B RAG 系统**

> **代码依旧是最高效的自然语言**

---

## 🎯 明天计划

### **优先级 1**
- [ ] 阅读完整 OpenSage 论文
- [ ] 安装 OpenViking 并测试
- [ ] 研究 MiroFish 的预测流程

### **优先级 2**
- [ ] 配置 MAS Factory 双模型策略
- [ ] 试用 Polymarket 交易工具
- [ ] 整理公开学习库

### **优先级 3**
- [ ] 研究 MagicSkills 集成
- [ ] 测试各种 AI 工具
- [ ] 撰写使用心得

---

**最后更新**: 2026-03-22 00:25
**总笔记数**: 38 篇
**Fork 项目**: 12 个
**GitHub**: https://github.com/srxly888-creator/masfactory-learning
